...
void generate_sat(const Mat& image, Mat& sat)
{
    ...
    // Vertical step
    satPtr = static_cast<SatType*>(sat.data) + 2 * sat.row_stride();
    ...
    for (int y = 2; y < sat.rows; ++y) {
        ...
        for (int x = 1; x < sat.cols; ++x) {
            for (int c = 0; c < channels; ++c) {
                *satPtr = *(satPtr - sat.row_stride()) + *satPtr;
                satPtr++;
            }
        }
    }
...
{
    assert(scale < 1.0f);
    assert(channels == source.channels());
    assert(source.type() == SatTypeEnum);

    int dstCols =
        static_cast<int>(round(scale * static_cast<float>(source.cols)));
    int dstRows =
    ...
    const float pixel_area_conj = 1.f / static_cast<float>(step_x * step_y);
    ...
    for (int dst_y = 0; dst_y < destination.rows; ++dst_y) {
        const double src_lower_y = step_y * static_cast<double>(dst_y);
        const double src_upper_y =
            clamp(src_lower_y + step_y, 0.0, source.rows - 1.0);

        for (int dst_x = 0; dst_x < destination.cols; ++dst_x) {
            const double src_lower_x = step_x * static_cast<double>(dst_x);
            const double src_upper_x =
                clamp(src_lower_x + step_x, 0.0, source.cols - 1.0);

            sample_sat(it_sat, src_lower_x, src_lower_y, Sa);

            sample_sat(it_sat, src_upper_x, src_lower_y, Sb);

            sample_sat(it_sat, src_upper_x, src_upper_y, Sc);

            sample_sat(it_sat, src_lower_x, src_upper_y, Sd);
            ...
            for (int c = 0; c < channels; ++c) {
                ...
                float area = (Sc[c] - Sb[c] - Sd[c] + Sa[c]) * pixel_area_conj;

                it_dst(dst_y, dst_x, c) = discretize_pixel<T>(area);
            }
        }
    }
}

template void generate_sat<1>(const Mat& image, Mat& sat);

template void generate_sat<3>(const Mat& image, Mat& sat);

template void
scale_from_sat<uint8_t, 1>(const Mat& source, float scale, Mat& destination);

template void
scale_from_sat<uint8_t, 3>(const Mat& source, float scale, Mat& destination);
